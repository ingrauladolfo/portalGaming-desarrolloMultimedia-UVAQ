...
class Figura {
  constructor(rotaciones) {
    ...
    const colorAleatorio =
      COLORES_PARA_ELEGIR[
        obtenerNumeroAleatorioEnRango(0, COLORES_PARA_ELEGIR.length - 1)
      ];
...
const sonidoFondo = cargarSonido(
...
const puntoEstaFueraDeLimites = (punto) => {
  ...
  return (
    xRelativo < 0 ||
    xRelativo > punto.limiteX ||
    yRelativo < 0 ||
    yRelativo > punto.limiteY
  );
};
const puntoAbsolutoFueraDeLimites = (xRelativo, yRelativo) => {
  return (
    xRelativo < 0 ||
    xRelativo > COLUMNAS - 1 ||
    yRelativo < 0 ||
    yRelativo > FILAS - 1
  );
};
const puntoValido = (puntoParaComprobar, posicionX, posicionY, puntos) => {
  const desocupado = puntoDesocupadoEnJuego(
    posicionX + puntoParaComprobar.x,
    posicionY + puntoParaComprobar.y
  );
  const ocupaMismaCoordenadaQuePuntoActual =
    puntos.findIndex((puntoExistente) => {
      return (
        puntoParaComprobar.x === puntoExistente.x &&
        puntoParaComprobar.y === puntoExistente.y
      );
    }) !== -1;
  const fueraDeLimites = puntoEstaFueraDeLimites(puntoParaComprobar);
  return !(
    (!desocupado && !ocupaMismaCoordenadaQuePuntoActual) ||
    fueraDeLimites
  );
};
/**
    ...
    for (const y of puntosInvertidos) {
      ...
      tablero = tablero.map((punto) => {
        if (punto.y < y) {
          let contador = 0;
          while (
            puntoDesocupadoEnJuego(punto.x, punto.y + 1) &&
            !puntoAbsolutoFueraDeLimites(punto.x, punto.y + 1) &&
            contador < puntos.length
          ) {
            punto.y++;
            contador++;
            sincronizarPiezasConTablero();
  ...
  if (j.puedeMoverAbajo(miY, miX)) {
    ...
    setTimeout(() => {
      ...
      if (pierde()) {
        Swal.fire(
          "Juego terminado",
          "Perdiste. Refresca la página para jugar de nuevo"
        );
        sonidoFondo.pause();
        puedeJugar = false;
        return;
...
<br>
Gracias a <a href="https://freesound.org/people/grunz/sounds/109662/">Freesound.org</a> por el sonido al completar una línea
`
);
